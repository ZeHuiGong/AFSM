...
from .post_process import (mstest_box_post_process, mstest_mask_post_process,
                           box_flip, mstest_corner_post_process)
...
def eval_run(exe,
    ...
    def recursive_sequence_lengths(v):
        return [[v.shape()[0]]] if v.shape()[1] == 6 else v.recursive_sequence_lengths()
    ...
    try:
        ...
        while True:
            outs = exe.run(compile_program,
                           ...
                           return_numpy=False)
            res = {
                k: (np.array(v), recursive_sequence_lengths(v))
            ...
            corner_ms_test = getattr(cfg, 'CornerMultiscaleTest', None)
            if corner_ms_test:
                post_res = mstest_corner_post_process(res)
                res.update(post_res)
                clean_keys = ['im_id', 'bbox', 'im_shape']
                if 'gt_bbox' in res:
                   clean_keys += ['gt_bbox', 'gt_class', 'is_difficult'] 
                res = clean_res(res, clean_keys)
            post_config = getattr(cfg, 'PostProcess', None)
            cor_or_cen = 'Corner' in cfg.architecture or 'Center' in cfg.architecture
            if cor_or_cen and post_config is not None:
