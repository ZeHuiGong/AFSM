...
def mstest_corner_post_process(result):
    """
    Corner or CenterNet multi-scale test
    Only available for batch_size=1 now.
    """
    post_bbox = {}
    detections = []
    for k in result.keys():
        if 'bbox' in k and k != 'gt_bbox':
            detections.append(result[k][0])
    detections = np.vstack(detections)
    idxes = np.argsort(-detections[:, 1], axis=0)
    detections = detections[idxes]
    post_bbox.update({'bbox': (detections, [[detections.shape[0]]])})
    return post_bbox
