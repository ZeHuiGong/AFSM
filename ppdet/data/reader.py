...
def _calc_img_weights_mosaic(roidbs):
    """get histogram of category distribution, and get
    probability of each sample for mosaic data augmentation"""
    show_time = defaultdict(int)
    imgs_cls = []
    img_weights = []
    for roidb in roidbs:
        result = Counter(roidb['gt_class'][:, 0])
        for t in result:
            show_time[t] += result[t]
        img_cls = set([k for cls in roidb['gt_class'] for k in cls])
        imgs_cls.append(img_cls)
    for i in range(len(roidbs)):
        weights = 0
        for c in imgs_cls[i]:
            weights += 1 / show_time[c]
        img_weights.append(weights)
    img_weights = img_weights / np.sum(img_weights)
    return img_weights
...
class Reader(object):
    ...
    def __init__(self,
        ...
        if inputs_def and inputs_def.get('multi_scale', False):
            from ppdet.modeling.architectures.input_helper import multiscale_def, corner_multiscale_def
            im_shape = inputs_def[
            ...
            #                               inputs_def['use_flip'])
            _, ms_fields = corner_multiscale_def(im_shape, inputs_def['test_scales'],
                                                 inputs_def['use_flip'])
        ...
        # both mixup and mosaic
        if self._mixup_epoch > 0 and self._mosaic_epoch > 0:
            self._mixup_method = 'both'
            assert self._mixup_epoch - self._mosaic_epoch != 0
            self._mixup_first = True
            if self._mixup_epoch - self._mosaic_epoch > 0:
                self._mixup_first = False
        elif self._mixup_epoch > 0:
            self._mixup_method = 'mixup'
        elif self._mosaic_epoch > 0:
            self._mixup_method = 'mosaic'
        else:
            self._mixup_method = 'none'
        ...
        if self._class_aware_sampling:
            self.img_weights = _calc_img_weights_mosaic(self._roidbs)
        if self._mixup_method in ['mosaic', 'both']:
            self.mosaic_img_weights = _calc_img_weights_mosaic(self._roidbs)
        self._indexes = None
    ...
    def reset(self):
        ...
        if self._mosaic_epoch > 0 and len(self.indexes) < 4:
            logger.debug("Disable mosaic for dataset samples "
                         "less than 4 samples")
            self._mosaic_epoch = -1
    ...
    def _load_batch(self):
        ...
        while bs != self._batch_size:
            ...
            if self._mixup_method == 'both':
                if self._mixup_first:
                    if self._epoch < self._mixup_epoch:
                        sample = self.load_mixup(sample)
                    elif self._epoch < self._mosaic_epoch:
                        sample = self.load_mosaic(sample)
                else:
                    if self._epoch < self._mosaic_epoch:
                        sample = self.load_mosaic(sample)
                    elif self._epoch < self._mixup_epoch:
                        sample = self.load_mixup(sample)
            elif self._mixup_method == 'mixup' and self._epoch < self._mixup_epoch:
                sample = self.load_mixup(sample)
            elif self._mixup_method == 'mosaic' and self._epoch < self._mosaic_epoch:
                sample = self.load_mosaic(sample)
    ...
    def load_mosaic(self, sample):
        choices = np.random.choice(self._sample_num, 3, p=self.mosaic_img_weights)
        mosaic = []
        for i, choice in enumerate(choices):
            mosaic.append(copy.deepcopy(self._roidbs[choice]))
            if self._load_img:
                mosaic[i]['image'] = self._load_image(mosaic[i]['im_file'])
        sample['mosaic'] = mosaic
        return sample
