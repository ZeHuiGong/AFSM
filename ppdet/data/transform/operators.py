...
from .mosaic_augment_utis import *
...
class DecodeImage(BaseOperator):
    def __init__(self, to_rgb=True, with_mixup=False, with_mosaic=False):
    ...
    def __call__(self, sample, context=None):
        ...
        if self.with_mosaic and 'mosaic' in sample:
            [self.__call__(mosaic_samp, context) for mosaic_samp in sample['mosaic']]
        return sample
...
class MosaicImage(BaseOperator):
    """executing Mosaic data augmentation
    Args:
        input_size (list, tuple): input image size (w, h)
    """
    def __init__(self,
                 input_size=[511, 511],
                 min_offset=0.2,
    ...
    def __call__(self, sample, context=None):
        if 'mosaic' not in sample:
            return sample
        mosaic_ims = sample['mosaic']
        # ws = [sample['image'].shape[1]] + [im['image'].shape[1] for im in mosaic_ims]
        # hs = [sample['image'].shape[0]] + [im['image'].shape[0] for im in mosaic_ims]
        # mosaic_w, mosaic_h = max(ws), max(hs)  # mosaic image size
        mosaic_w, mosaic_h = self.input_w, self.input_h

        cut_x = random.randint(int(mosaic_w * self.min_offset), int(mosaic_w * (1 - self.min_offset)))
        cut_y = random.randint(int(mosaic_h * self.min_offset), int(mosaic_h * (1 - self.min_offset)))

        out_img = np.zeros([mosaic_h, mosaic_w, 3])
        out_bboxes = []
        ...
        for i in range(4):
            if i == 0:
                ...
                mosaic_im = mosaic_ims[i - 1]
            gt_bbox = np.concatenate([mosaic_im['gt_bbox'], mosaic_im['gt_class'],
                                      mosaic_im['gt_score'], mosaic_im['is_crowd']], axis=1)
            oh, ow, oc = mosaic_im['image'].shape
            ...
            if self.blur:
                tmp_blur = random.randint(0, 1)  # 0 - disable, 1 - blur background, 2 - blur the whole image
                if tmp_blur == 0:
                    blur = 0
                elif tmp_blur == 1:
                    blur = self.blur
            ...
            gt_bbox, min_w_h = fill_truth_detection(gt_bbox, pleft, ptop, swidth, sheight, mosaic_w, mosaic_h)
            if (min_w_h / 8) < blur:  # disable blur if one of the objects is too small
                blur = min_w_h / 8

            aug_img = image_data_augmentation(
                mosaic_im['image'], gt_bbox, mosaic_w, mosaic_h, pleft, ptop, swidth, sheight, blur=blur)

            left_shift = int(min(cut_x, max(0, (-pleft * mosaic_w / ow))))
            top_shift = int(min(cut_y, max(0, (-ptop * mosaic_h / oh))))

            right_shift = int(min((mosaic_w - cut_x), max(0, (-pright * mosaic_w / ow))))
            bot_shift = int(min(mosaic_h - cut_y, max(0, (-pbot * mosaic_h / oh))))

            out_img, gt_bbox = blend_truth_mosaic(
                out_img, aug_img, gt_bbox, mosaic_w, mosaic_h,
                cut_x, cut_y, i, left_shift, right_shift, top_shift, bot_shift)

            out_bboxes.append(gt_bbox)
        bboxes = np.concatenate(out_bboxes, axis=0)
        sample['image'] = out_img
        sample['gt_bbox'] = bboxes[:, :4].astype(np.float32)
        sample['gt_class'] = bboxes[:, 4:5].astype(np.int32)
        sample['gt_score'] = bboxes[:, 5:6].astype(np.float32)
        sample['is_crowd'] = bboxes[:, 6:7].astype(np.int32)
        sample['h'] = out_img.shape[0]
        sample['w'] = out_img.shape[1]
        sample.pop('mosaic')
...
class Resize(BaseOperator):
    ...
    Args:
        target_dim (list or list[tuple]): target size, can be a single number or a list
            (for random shape).
    ...
    def __call__(self, sample, context=None):
        ...
        if mmcv.is_list_of(self.target_dim, tuple):
            dim_w, dim_h = random.choice(self.target_dim)
        else:
            dim_w, dim_h = self.target_dim
        resize_w, resize_h = (dim_w, dim_h)
        scale_x = dim_w / w
        scale_y = dim_h / h
        if 'gt_bbox' in sample and len(sample['gt_bbox']) > 0:
            scale_array = np.array([scale_x, scale_y] * 2, dtype=np.float32)
            sample['gt_bbox'] = sample['gt_bbox'] * scale_array
            sample['gt_bbox'][:, 0:4:2] = np.clip(sample['gt_bbox'][:, 0:4:2],
                                                  0, dim_w - 1)
            sample['gt_bbox'][:, 1:4:2] = np.clip(sample['gt_bbox'][:, 1:4:2],
                                                  0, dim_h - 1)
        sample['h'] = resize_h
        sample['w'] = resize_w
...
@register_op
class CenterTarget(BaseOperator):
    """
    Generate targets for CenterNet(Object as Points) by ground truth data.
    Args:
        output_size (int): the size of output heatmaps.
        num_classes (int): num of classes.
        gaussian_bump (bool): whether to apply gaussian bump on gt targets.
        ...
        gaussian_iou (float): the threshold iou of predicted bbox to gt bbox.
            ...
            positive sample. 0.3 by default
        max_tag_len (int): max num of gt box per image.
        with_mixup: (bool): a flag that determines whether use mixup augmentation,
            if True, the loss value have to be weighted according to the mixup parameter lambda.
    """
    ...
    def __init__(self,
                 ...
                 gaussian_iou=0.3,
                 max_tag_len=128):
        super(CenterTarget, self).__init__()
        self.num_classes = num_classes
    ...
    def __call__(self, sample, context=None):
        heatmaps = np.zeros(
            ...
            dtype=np.float32)
        wh = np.zeros((self.max_tag_len, 2), dtype=np.float32)
        regrs = np.zeros((self.max_tag_len, 2), dtype=np.float32)
        ind = np.zeros((self.max_tag_len), dtype=np.int64)
        tag_masks = np.zeros((self.max_tag_len), dtype=np.float32)
        heat_weight = np.zeros(
        ...
        gt_bbox = gt_bbox[keep_inds]
        gt_class = gt_class[keep_inds]
        gt_score = gt_score[keep_inds]
        sample['gt_bbox'] = gt_bbox
        sample['gt_class'] = gt_class
        sample['gt_score'] = gt_score
        width_ratio = self.output_w / sample['w']
        height_ratio = self.output_h / sample['h']
        # loop through each ground-truth box
        obj_num = min(self.max_tag_len, gt_bbox.shape[0])
        for i in range(obj_num):
            ...
            fct_x = (xtl + xbr) * width_ratio / 2
            fct_y = (ytl + ybr) * height_ratio / 2
            ct_x = int(fct_x)
            ct_y = int(fct_y)
            if self.gaussian_bump:
                ...
                if self.gaussian_rad == -1:
                    radius = gaussian_radius(
                        (math.ceil(height), math.ceil(width)), self.gaussian_iou)
                    radius = max(0, int(radius))
                else:
                    radius = self.gaussian_rad
                draw_gaussian(heatmaps[gt_class[i][0]], [ct_x, ct_y], radius)
            else:
                heatmaps[gt_class[i][0], ct_y, ct_x] = 1
            heat_weight[gt_class[i][0], ct_y, ct_x] = gt_score[i, 0]
            wh[i, :] = [width, height]
            regrs[i, :] = [fct_x - ct_x, fct_y - ct_y]
            ind[i] = ct_y * self.output_w + ct_x
            tag_masks[i] = gt_score[i, 0]

        sample['heatmaps'] = heatmaps
        sample['regrs'] = regrs
        sample['ind'] = ind
        sample['reg_mask'] = tag_masks
        sample['wh'] = wh
...
@register_op
class CenterTargetIOU(BaseOperator):
    """
    Generate targets for CenterNet(Object as Points) by ground truth data.
    predicting the width and height using the features at the center, and 
    use iou loss for w/h regression. (e.g. GIOU/DIOU)
    Args:
        output_size (int): the size of output heatmaps.
        num_classes (int): num of classes.
        gaussian_bump (bool): whether to apply gaussian bump on gt targets.
    ...
    def __init__(self,
                 ...
                 gaussian_iou=0.3,
                 max_tag_len=128):
        super(CenterTargetIOU, self).__init__()
        self.num_classes = num_classes
        self.output_w, self.output_h = output_size
        self.gaussian_bump = gaussian_bump
        self.gaussian_rad = gaussian_rad
    ...
    def __call__(self, sample, context=None):
        heatmaps = np.zeros(
            ...
            dtype=np.float32)
        regrs = np.zeros((self.max_tag_len, 2), dtype=np.float32)
        tlbr = np.zeros((2, self.output_h, self.output_w), dtype=np.float32)
        tlbr_mask = np.zeros((1, self.output_h, self.output_w), dtype=np.float32)
        ind = np.zeros((self.max_tag_len, ), dtype=np.int64)
        tag_masks = np.zeros((self.max_tag_len, ), dtype=np.uint8)
        ...
        for i in range(obj_num):
            ...
            tlbr[:, ct_y, ct_x] = [width, height]
            tlbr_mask[0, ct_y, ct_x] = 1
...
class CornerCrop(BaseOperator):
    ...
    Args:
        ...
        input_size (list, tuple): size of input image
    """
    ...
    def __call__(self, sample, context=None):
        ...
        if self.is_train:
            ...
            height = int(self.input_h * scale)
            width = int(self.input_w * scale)
            ...
            height = im_h | (self.border - 1)
            width = im_w | (self.border - 1)
...
class CornerRatio(BaseOperator):
    ...
    Args:
        input_size (list, tuple): the size of input size
        output_size (list, tuple): the size of heatmap
    """
    ...
    def __call__(self, sample, context=None):
        scale = (self.input_size[0] + 1) // self.output_size[0]
...
@register_op
class CornerMultiscaleTest(BaseOperator):
    """
           Rescale image to the each size in target size, and capped at max_size.

           Args:
               test_scales (list): teseing scales
               interp (int): the interpolation method.
               use_flip (bool): whether use flip augmentation.
           """
    ...
    def __init__(self,
                 ...
                 interp=cv2.INTER_LINEAR,
                 border=128):
        super(CornerMultiscaleTest, self).__init__()
        self.test_scales = test_scales
        self.use_flip = use_flip
        self.interp = interp
        self.border = border

        if not isinstance(test_scales, list):
            raise TypeError(
                "Type of target_size is invalid. Must be List, now is {}".
                format(type(test_scales)))
    ...
    def __call__(self, sample, context=None):
        ...
        for i, scale in enumerate(self.test_scales):
            ...
            new_center = (new_height // 2, new_width // 2)
            inp_height = new_height | (self.border - 1)
            inp_width = new_width | (self.border - 1)
            ...
            resized_image, border, offset = corner_crop_image(
            ...
            sample[im_name] = resized_image
            if self.use_flip:
                im_name_flip = 'image_flip_scale_{}'.format(scale)
                sample[im_name_flip] = resized_image[:, ::-1, :]
            sample[ratios_name] = np.array([height_ratio, width_ratio])
            sample[border_name] = border
            sample[sizes_name] = np.array(
                [int(height * scale), int(width * scale)], dtype=np.float32)
        return sample
